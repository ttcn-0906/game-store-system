...
screen -dmS $SCREEN_NAME -t database bash -c "source $VENV_NAME/bin/activate && cd server && python db.py; exec bash"

sleep 2 # Wait for DB

# 4. Create a new window (window 1) for Developer Server
echo "-> Starting Developer Server in Window 1..."
screen -S $SCREEN_NAME -X screen -t developer bash -c "source $VENV_NAME/bin/activate && cd server && python developer_server.py; exec bash"

sleep 1

# 5. Create a new window (window 2) for Player Server
echo "-> Starting Player Server in Window 2..."
screen -S $SCREEN_NAME -X screen -t player bash -c "source $VENV_NAME/bin/activate && cd server && python player_server.py; exec bash"
